...
const apiUpdateOrderStatus = function (data) {
  return axios.get(`/api/order/admin/update-order-status/`, { order_id: data.id, order_status: data.orderStatus });
};

const apiUpdatePaymentStatus = function (data) {
  return axios.get(`/api/order/admin/update-payment-status/`, { order_id: data.id, order_status: data.paymentStatus });
};
